...
Measure claim and completion behavior with growing processed history. The initial hypothesis proposed 10,000, 100,000, and 1,000,000 rows; measured setup cost determines whether each larger checkpoint adds useful V1 evidence.
...
**TE-L05-B executable:** `.\operations\Run-StorageStateMeasurements.ps1` creates independent 5,000-row pending, actively claimed, processed, and terminally failed populations with 1 KB of the same representative content. Processing, completion, and all three rejected attempts for failed rows run through real worker processes. SQL Server measured approximately 4.29 KB, 4.42 KB, 4.52 KB, and 4.57 KB total allocated bytes per row respectively. PostgreSQL measured approximately 1.48 KB, 2.93 KB, 1.95 KB, and 2.34 KB. A batch of 10 completed both provider runs with exact state and effect counts under the deliberately short five-second dogfood claim.

**TE-L05-C executable:** `.\operations\Run-RetainedHistoryLoad.ps1` creates independent processed histories through the real publisher and four real workers, then drains the same 1,000-message active backlog with batch 10. SQL Server measured 204.19, 190.78, and 187.85 messages per second after 0, 10,000, and 100,000 retained rows: the largest history preserved 92% of empty-history throughput. PostgreSQL measured 259.37, 238.08, and 226.43 messages per second, preserving 87.3%. Every variant reached exact terminal counts with zero failure, duplicate, or lease loss. A lightweight provider-specific existence probe checks only whether pending or processing work remains; the complete evidence model is read once at the terminal boundary. Its 250-millisecond interval is the maximum measurement-boundary distortion. These local values are comparative evidence, not universal capacity guarantees.
...
**TE-L06-C1 executable:** the same runner prepares 1,000 eligible processed rows and starts the real cleanup background service with a 37-row batch. After durable partial progress, it terminates that process abruptly and proves the remaining count is stable while no cleaner exists. A replacement process then removes the exact durable remainder while all 1,000 business rows survive. SQL Server and PostgreSQL pass from Dogfood commit `940f395` and TinyEvents `main` commit `52e6889`. This proves replacement-process recovery only; C2 addresses same-process recovery after database interruption separately.

**TE-L06-C2 executable:** the same runner interrupts the real database after the background service has made partial progress over 1,000 eligible rows. The cleanup-specific logs must show bounded failures while the process remains alive, then recovery after the database returns. A debug-level product event proves the same process deletes another batch after recovery before the durable outbox reaches zero. SQL Server and PostgreSQL pass from Dogfood commit `7113f60` and TinyEvents `main` commit `52e6889`, each with 963 rows remaining when the outage began. Together, C1 and C2 close cleanup recovery; active-load interference remains separate.

**TE-L06-D executable:** `.\operations\Run-CleanupUnderLoad.ps1` compares isolated cleanup-disabled and candidate-policy variants at 200, 400, and 800 requests per second. Every variant begins with 50,000 eligible processed rows, starts one real transaction per publishing request, and runs four real worker processes with a maximum pool of 16 connections per process. SQL Server and PostgreSQL pass from Dogfood commit `fe447e8` and TinyEvents commit `c629baf`. Candidate throughput remained within 0.2% of baseline at 200 and 400 on both providers; PostgreSQL also sustained 800 with comparable p95. SQL Server was beyond its local sustainable publishing rate at 800 in both variants, and candidate p95 reached 420.13 ms with an 11.01-second settlement tail. All active messages nevertheless completed exactly once, every worker participated, and candidate cleanup removed 40,000 to 50,000 eligible rows while publishing remained active. These are local interference measurements for the policy decision, not universal capacity guarantees.

The first PostgreSQL TE-L06-D attempt used the provider's default 100-connection pool independently in every process against a server limited to 100 total connections. It failed baseline and candidate variants with PostgreSQL `53300` rather than hiding lost requests. The accepted runner reuses the explicit 16-connection per-process budget from TE-L03 and records it in the manifest; it does not increase the server limit.

**TE-L06-E decision:** the next release keeps one-hour processed retention, a 1,000-row batch, and a one-second interval. One instance therefore has nominal configured cleanup capacity up to the highest tested 800-message-per-second target, while the values remain adjustable for smaller database budgets or lower cleanup pressure. Applying TE-L05-B's representative processed-row measurements to one hour gives 3.25/6.50/13.00 decimal GB on SQL Server and 1.40/2.81/5.62 GB on PostgreSQL at 200/400/800 messages per second. These projections cover the measured outbox table and indexes only; they are planning evidence, not universal database-size guarantees. TinyEvents commit `936794c` publishes the formula, limitations, and accepted defaults. TE-L06 is complete.

#### TE-L07 - Soak and repeated disruption

**TE-L07 executable:** `.\operations\Run-DisruptionSoak.ps1` runs the same bounded 120-second mixed workload against SQL Server and PostgreSQL with four processing-and-cleanup workers, 50,000 expired processed rows, two claimed-worker deaths and replacements, and two five-second database outages. Both providers pass from Dogfood commit `a816be2` and TinyEvents commit `936794c`. SQL Server acknowledged 17,530 requests and contained 17,533 durable current operations, explicitly retaining three commits whose acknowledgement was lost during connection interruption. PostgreSQL acknowledged and durably contained 21,873. Every durable operation reached its expected terminal state, every processed operation had one distinct effect, both replacement workers participated, all seeded history was removed, and neither provider retained pending or processing work.

Each run observed three duplicate effects at deliberate process-death boundaries and one consumer failure whose dogfood side effect survived while its subsequent outbox failure update was interrupted. Those are retained at-least-once evidence, not hidden as exactly-once behavior. The runner bounds the latter gap by the number of active processes deliberately terminated and reconciles database commits independently from publisher acknowledgements. Peak observed database connections were 29 for both providers; the largest sampled individual process working set was about 141 MB on SQL Server and 145 MB on PostgreSQL. Resource values remain local evidence rather than product limits. TE-L07 is complete.
...
TE-S01 starts eight independent application migrators against a fresh SQL Server or PostgreSQL database. Every process completes successfully while the durable result contains one outbox table and the exact ordered current migration history. This proves process-level migration serialization in addition to each provider's real-database integration tests.

The process-level scenario passed against both providers on 2026-08-24: one process applied migrations `001_CreateTinyOutbox` and `002_AddProcessedCleanupIndex`, seven observed the current schema, and durable history contained exactly those two migrations.
...
TE-S02-A restores the package-consuming upgrade host only from nuget.org and runs it against published `0.1.0-alpha.3`. The SQL Server acceptance run creates one pending message, one processing message with an expired lease, and one terminally failed message with one attempt. All three rows retain one exact event type, the alpha schema contains migration `001`, and no candidate consumer effect exists.

TE-S02-B packs the release train from clean `main`, restores the same package-only host from those candidate packages, migrates the alpha-created database, and invokes the real outbox processor. The candidate processes the pending and expired-processing rows with exactly two distinct effects, preserves the terminally failed row, its single attempt, and its exact error, and advances migration history from one alpha row to the two-row current schema.

TE-S02-C runs that unchanged package-only contract against PostgreSQL. On 2026-08-24, candidate TinyEvents `main` commit `1feb235` advanced both providers from the published alpha's migration `001` through `002_AddProcessedCleanupIndex`, processed two supported rows with two distinct effects, and preserved the exact terminal failure. Both providers passed together, so `TE-S02` is complete.

TE-S03 uses a temporary SQL Server DDL trigger or PostgreSQL event trigger to block the real migrator after it owns the provider migration lock. The runner observes that boundary from the database, terminates the application process, waits for the abandoned lock to disappear, removes the fault, and starts a replacement process. Acceptance allows any atomic resumable boundary after process death: no migration infrastructure, empty history infrastructure, or a completely committed schema. Mixed outbox/history state is rejected. Both providers passed on 2026-08-24 and recovered to the exact ordered two-migration history.

TE-S04 prepares three durable schema states through provider-specific external setup. A completely absent schema migrates to the exact current history. Current history without its physical outbox is rejected as inconsistent and names the missing table. A checksum-conflicting migration is rejected with its version and checksum diagnostic. The initial run exposed that both providers silently accepted the missing outbox; TinyEvents fix `5396617` closed that hole. SQL Server and PostgreSQL passed the two-migration scenario against TinyEvents `main` commit `37c960d` on 2026-08-24.

TE-S05 builds package-only application assemblies against published `0.1.0-alpha.3` and clean `main`, creates a separate 100-message alpha backlog, and starts the alpha worker first. Durable state must prove alpha participation before candidate starts and applies migration `002`. This models an actual rolling deployment and removes the invalid race between two cold-start migrators. Acceptance requires both versions to participate, every message to produce one distinct operation effect, and migration history to advance from one alpha row to the two-row current schema. An alpha process restarted after `002` fails fast because it cannot prove compatibility with a future schema; restarted instances must use candidate. On 2026-08-24, both providers processed all 100 messages with both worker identities, 100 distinct operation effects, no remaining or failed work, and exactly two migrations against candidate TinyEvents `main` commit `1feb235`.
