...
| `TE-L04` | `.\operations\Run-BacklogRecoveryLoad.ps1` | One publisher first creates at least 1,000 pending messages at 200 requests per second. Four workers then reduce outstanding work to no more than one second of incoming traffic while that publisher remains active. All 6,000 messages subsequently complete once with every worker participating. |
...
| `TE-S02` | `.\deployment\Run-PublishedAlphaUpgrade.ps1` | Published `0.1.0-alpha.3` packages create pending, reclaimable-processing, and failed state with migration `001`. Clean-main candidate packages advance both providers through migration `002`, process supported work exactly once in the lab, and preserve the terminal failure. |
...
| `TE-S05` | `.\deployment\Run-RollingUpgrade.ps1 -CandidateRoot <clean-main-path>` | A running published-alpha worker remains active while clean-main migrates through `002` and joins the shared backlog. Both versions participate while every message produces one distinct durable effect. |
