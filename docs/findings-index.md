...
| Live backlog recovery was coupled to a narrow publisher-duration race and intrusive complete observations. | TE-L04 uses an outbox-only outstanding count, takes exact evidence at the recovery boundary, and allows 30 seconds at the unchanged 200 requests per second. Acceptance still requires recovery to one second of incoming work before publishing stops. |
| The timed-worker fixture started its lifetime clock before host startup completed. | `worker-for` now starts the host before measuring its requested run duration, so TE-W08 tests graceful shutdown after startup rather than canceled startup. TinyEvents' explicit canceled-start contract remains unchanged. |
