...
- [ ] `TE-S02` — Create representative in-flight state with the published `0.1.0-alpha.3` packages, upgrade to the beta candidate, migrate, and process supported messages.
  ...
  - [ ] `TE-S02-B` — Pack the candidate from clean `main`, migrate the alpha database, drain supported work, and preserve the failed row.
  - [ ] `TE-S02-C` — Run the unchanged upgrade contract against PostgreSQL.
