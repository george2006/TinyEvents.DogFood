...
Processed outbox rows are intentionally retained by the existing scenarios unless a cleanup scenario explicitly enables deletion. `TE-L05` measured payload curves, physical cost by status, and active drain behavior through 100,000 retained rows. Cleanup is now implemented in TinyEvents `main`; `TE-L06` must validate its candidate retention, deletion-batch, and storage-budget decisions before beta acceptance.
